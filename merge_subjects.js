...
class SubjectMerger {
  ...
  loadPensumData(pensumPath) {
    ...
    this.programs.forEach((program) => {
      ...
      console.log(
        `📖 Cargando pensum: ${program.name} (${program.pensumFile})`,
      );
      ...
      if (
        !pensumData ||
        !pensumData.materias ||
        !Array.isArray(pensumData.materias)
      ) {
      ...
      console.log(
        `   ✅ ${pensumData.materias.length} materias cargadas del pensum`,
      );
      loadedPensums++;
  ...
  processSubject(subject, programInfo) {
    ...
    if (this.subjectsMap.has(sku)) {
      ...
      if (subject.groups) {
        const existingGroupSkus = new Set(
          existingSubject.groups.map((g) => g.sku),
        );
  ...
  updateWithPensumData() {
    ...
    this.subjectsMap.forEach((subject, sku) => {
      ...
      if (pensumInfo) {
        ...
        if (
          (!subject.requirements || subject.requirements.length === 0) &&
          pensumInfo.requisitos &&
          pensumInfo.requisitos.length > 0
        ) {
  ...
  saveResult(outputPath) {
    try {
      ...
      console.log(
        `   Materias compartidas entre programas: ${stats.sharedSubjects}`,
      );
      console.log(`   Materias con créditos: ${stats.subjectsWithCredits}`);
      console.log(
        `   Materias con requisitos: ${stats.subjectsWithRequirements}`,
      );
      console.log("\n   Distribución por programas:");
      Object.keys(stats.subjectsByPrograms)
        .sort()
        .forEach((count) => {
          console.log(
            `   ${count} programa(s): ${stats.subjectsByPrograms[count]} materias`,
          );
        });
  ...
  showSharedSubjectsExamples(limit = 5) {
    ...
    for (const [sku, subject] of this.subjectsMap) {
      if (subject.program.length > 1 && count < limit) {
        ...
        console.log(
          `   Requisitos: ${subject.requirements.length > 0 ? subject.requirements.join(", ") : "Ninguno"}`,
        );
